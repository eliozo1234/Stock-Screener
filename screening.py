...
def search_stocks():
    ...
    try:
        ...
        volume_period = data.get('volume_period', 30)  # 30, 90 ou 180 jours
        ...
        # Exclure les IPO récentes (moins d'un an)
        one_year_ago = datetime.now().date() - timedelta(days=365)
        query = query.filter(
            (Ticker.ipo_date.is_(None)) | (Ticker.ipo_date <= one_year_ago)
        )
        ...
        for ticker in tickers:
            ...
            if pct_of_high <= threshold_pct:
                # Calculer le volume moyen sur la période spécifiée
                avg_volume_record = db.session.query(
                    func.avg(Price.volume).label('avg_volume')
                ).filter(
                    and_(
                        Price.ticker_id == ticker.id,
                        Price.date >= datetime.now().date() - timedelta(days=volume_period)
                    )
                ).first()
                ...
                if avg_volume >= min_volume:
                    results.append({
                        ...
                        'avg_volume_{}d'.format(volume_period): avg_volume
                    })
